...
const messageIndexProjectionDefinition = {
  ...
  apply: (state, event) => {
    ...
    if (event.type === "user/message") {
      ...
      const entry = { seq: event.seq, time: event.time, text, reply: "", ...(typeof data.id === "string" ? { id: data.id } : {}) };
      return { messages: [...state.messages, entry] };
    }
    // Reply digest: assistant text blocks accumulate onto the latest user
    // message until the next user turn (80-char cap, matching text).
    if (event.type === "assistant/message") {
      const data = event.data;
      if (data === null || typeof data !== "object" || data.message === null || typeof data.message !== "object") return state;
      if (state.messages.length === 0) return state;
      const chunk = textOf(data.message.content).slice(0, 80);
      if (chunk === "") return state;
      const last = state.messages[state.messages.length - 1];
      const merged = last.reply === "" ? chunk : (last.reply + " " + chunk).slice(0, 80);
      const next = state.messages.slice();
      next[next.length - 1] = { ...last, reply: merged };
